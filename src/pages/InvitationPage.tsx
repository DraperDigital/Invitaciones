        ...
        <section id="guest_welcome" key="guest_welcome" className="py-24 bg-[var(--section-bg)] border-b border-[var(--border-color)]">
            <div className="max-w-4xl mx-auto px-6 text-center">
                <div className="mb-12 inline-flex items-center justify-center">
                    <div className="h-px w-20 sm:w-28 bg-accent/30" />
                </div>
                ...
                <>
                    ...
                    >
                        ...
                        <div className="absolute right-16 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center gap-3 bg-stone-950/95 text-white p-2 rounded-2xl shadow-2xl border border-white/10 backdrop-blur-md whitespace-nowrap pointer-events-none animate-in fade-in slide-in-from-right-2 duration-200">
                            ...
                            <img src={nextDemo.thumbnail} alt={nextDemo.name} className="w-10 h-10 rounded-xl object-cover border border-white/10" />
                        </div>
                        <div className="w-12 h-12 rounded-full bg-stone-900/80 hover:bg-stone-900 text-white backdrop-blur-md border border-white/20 shadow-2xl flex items-center justify-center transition-all group-hover:scale-110 group-hover:border-[#DF3B94]">
                    ...
                    {/* Menú Lateral / Drawer para Seleccionar el Modelo a Ver */}
                    {showTemplateMenu && (
                        <div className="fixed inset-0 z-[1000] flex justify-end animate-in fade-in duration-200">
                            {/* Backdrop oscurecido */}
                            <div 
                            ...
                            <div className="relative w-full max-w-md bg-stone-900 text-white h-full shadow-2xl flex flex-col z-10 border-l border-white/10 overflow-hidden animate-in slide-in-from-right duration-300">
                                {/* Header del Drawer */}
                                <div className="p-5 border-b border-white/10 flex items-center justify-between bg-stone-950/70">
                                    <div>
                                        <div className="flex items-center gap-2">
                                        ...
                                        return (
                                            <Link
                                                ...
                                                className={`flex items-center gap-3.5 p-3 rounded-2xl border transition-all no-underline ${
                                                    isCurrent 
                                                        ? 'bg-[#DF3B94]/15 border-[#DF3B94] shadow-md ring-1 ring-[#DF3B94]/40' 
                                                        : 'bg-stone-800/60 hover:bg-stone-800 border-white/5 hover:border-white/20'
                                                }`}
                                            >
                                                <div className="relative w-14 h-14 rounded-xl overflow-hidden shrink-0 border border-white/10">
                                                    ...
                                                    <span className="absolute bottom-1 right-1 text-xs">{tpl.icon}</span>
                                                </div>

                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center justify-between gap-1">
                                                        <h4 className={`text-xs font-bold truncate ${isCurrent ? 'text-[#DF3B94]' : 'text-white'}`}>
                                                            {tpl.name}
                                                        </h4>
                                                        <span className="text-[9px] font-mono text-stone-400 bg-black/40 px-1.5 py-0.5 rounded">
                                                            #{idx + 1}
                                                        </span>
                                                    </div>
                                                    <p className="text-[10px] text-stone-400 mt-0.5 truncate">{tpl.categoryLabel}</p>
                                                    
                                                    {isCurrent ? (
                                                        <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-400 mt-1">
                                                            <CheckCircle2 className="h-3 w-3" /> Viendo ahora
                                                        </span>
                                                    ) : (
                                                        <span className="text-[10px] text-stone-400 hover:text-stone-200 mt-1 block">
                                                            Ver ejemplo en vivo →
                                                        </span>
                                                    )}
                                                </div>
                                ...
                                {/* Footer del Drawer */}
                                <div className="p-4 border-t border-white/10 bg-stone-950/70 flex items-center justify-between gap-3">
                                    <Link 
                                        to="/ejemplos" 
                                        onClick={() => setShowTemplateMenu(false)}
                                        className="text-xs text-stone-400 hover:text-white transition-colors flex items-center gap-1.5 no-underline"
                                    >
                                    ...
                                    <Link
                                        ...
                                        onClick={() => setShowTemplateMenu(false)}
                                        className="px-4 py-2 rounded-xl text-xs font-bold bg-[#DF3B94] hover:bg-[#C52A7C] text-white shadow-md transition-all active:scale-95 no-underline"
                                    >
                                        Comenzar con {currentDemo.name}
                                    </Link>
                                </div>
                            </div>
                        </div>
